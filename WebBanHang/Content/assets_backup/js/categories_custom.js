			...
			filterButton.on('click', function () {
				...
				$('.product-grid').isotope({
					filter: function () {
						...
						var priceMin = parseFloat($('#FromAmount').val());
						var priceMax = parseFloat($('#ToAmount').val());
						var itemPrice = parseFloat($(this).find('.in_product_price').text().trim());
	...
	*/
	...
	function initPriceSlider() {
		...
		if ($("#slider-range").length) {
			$("#slider-range").slider(
				{
					...
					slide: function (event, ui) {
						$("#amount").val("đ" + ui.values[0].toLocaleString('vi-VN') + " - đ" + ui.values[1].toLocaleString('vi-VN'));
			...
			$("#amount").val("đ" + $("#slider-range").slider("values", 0).toLocaleString('vi-VN') + " - đ" + $("#slider-range").slider("values", 1).toLocaleString('vi-VN'));
			$('#FromAmount').val($("#slider-range").slider("values", 0));
			$('#ToAmount').val($("#slider-range").slider("values", 1));
			
			console.log('Price slider initialized with values: ' + $("#slider-range").slider("values", 0) + ' - ' + $("#slider-range").slider("values", 1));
		} else {
			console.log('Slider-range element not found');
		}
	}
